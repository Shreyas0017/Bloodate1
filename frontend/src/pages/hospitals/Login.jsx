...
export default function HospitalLogin() {
  ...
  const [form, setForm] = useState({ email: 'hospital@bloodate.test', password: 'password' })
  const [error, setError] = useState('')
  const navigate = useNavigate()

  useEffect(() => {
    if (user && role === 'HOSPITAL_ADMIN') {
      navigate('/hospitals/landing', { replace: true })
    }
  }, [user, role, navigate])

  useEffect(() => {
    if (user && role && role !== 'HOSPITAL_ADMIN') {
      setError('Not authorized for hospital access')
      logout()
    }
  }, [user, role, logout])
  ...
  return (
    <div className="panel p-8 max-w-xl mx-auto">
      ...
      <p className="text-sm text-slate-500 mb-6">Access hospital tools for requests and uploads.</p>
